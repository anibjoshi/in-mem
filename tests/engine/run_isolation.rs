...
fn kv_runs_are_isolated() {
    ...
    assert_eq!(kv.get(&run_a, "key").unwrap().unwrap(), Value::Int(1));
    assert_eq!(kv.get(&run_b, "key").unwrap().unwrap(), Value::Int(2));
}
...
fn eventlog_independent_per_run() {
    ...
    assert_eq!(event.len(&run_a).unwrap(), 5);
    assert_eq!(event.len(&run_b).unwrap(), 5);
...
fn jsonstore_count_per_run() {
    ...
    assert_eq!(json.list(&run_a, None, None, 1000).unwrap().doc_ids.len(), 2);
    assert_eq!(json.list(&run_b, None, None, 1000).unwrap().doc_ids.len(), 1);
}
...
fn vectorstore_collections_per_run() {
    ...
    let a_colls = vector.list_collections(run_a).unwrap();
    let b_colls = vector.list_collections(run_b).unwrap();
...
fn all_primitives_isolated_by_run() {
    ...
    assert!(prims.state.read(&run_b, "cell").unwrap().is_none());

...
fn collection_exists(vector: &VectorStore, run_id: strata_core::RunId, name: &str) -> bool {
    vector
...
fn create_collection() {
    ...
    assert!(collection_exists(&vector, test_db.run_id, "test_collection"));
}
...
fn delete_collection() {
    ...
    assert!(collection_exists(&vector, test_db.run_id, "to_delete"));
...
fn count_vectors() {
    ...
    let get_count = || -> usize {
        vector
            .list_collections(test_db.run_id)
            .unwrap()
            .iter()
            .find(|c| c.name == "coll")
            .map(|c| c.count)
...
fn search_returns_similar_vectors() {
    ...
    let results = vector.search(test_db.run_id, "coll", &query, 2, None).unwrap();
...
fn search_respects_k_limit() {
    ...
    let results = vector.search(test_db.run_id, "coll", &query, 3, None).unwrap();
...
fn search_empty_collection() {
    ...
    let results = vector.search(test_db.run_id, "coll", &query, 5, None).unwrap();

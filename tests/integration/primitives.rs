...
mod kv_single {
    ...
    fn value_types() {
        ...
        assert!(matches!(kv.get(&run_id, "int").unwrap().unwrap(), Value::Int(42)));
        assert!(matches!(kv.get(&run_id, "bool").unwrap().unwrap(), Value::Bool(true)));
    }
}
...
mod event_single {
    ...
    fn multiple_streams() {
        ...
        assert_eq!(event.read_by_type(&run_id, "stream_a").unwrap().len(), 2);
        assert_eq!(event.read_by_type(&run_id, "stream_b").unwrap().len(), 1);
    }
...
fn cross_primitive_workflow_agent_memory() {
    ...
    assert_eq!(
        p.vector.list_collections(run_id).unwrap().iter()
